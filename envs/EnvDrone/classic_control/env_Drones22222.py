...
class SearchGrid(gym.Env):
    ...
    def step(self, action):
        ...
        # print("environment action is",action)
    ...
    def state_action_reward_done(self):  # 这里返回状态值，奖励值，以及游戏是否结束
        ...
        for i, single_map in enumerate(single_map_set):
            ...
            self.average_list[i] = max(self.find_grid_count[i] - self.last_find_grid_cout[i] \
        ...
        for i in range(self.drone_num - 1):  # 如果机器人发生碰撞
            for j in range(i + 1, self.drone_num):
                ...
                if distance <= 1:
                    ...
                    reward_list[i] -= collision_factor - min(collision_decay, self.MC_iter)
                    reward_list[j] -= collision_factor - min(collision_decay, self.MC_iter)
