...
class SearchGrid(gym.Env):
    def __init__(self, map_set, map_num):
        ...
        self.observation_space = spaces.Box(low=0, high=1, shape=(4 * 60 * 60,))
    ...
    def state_action_reward_done(self):  # 这里返回状态值，奖励值，以及游戏是否结束
        ...
        reward_list = list(map(lambda x: reward_scale * (x - min(num_duplicates*overlap_factor, abs(x)/2)), reward_list))  # 单步惩罚
