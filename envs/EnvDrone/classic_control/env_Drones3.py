...
class SearchGrid(gym.Env):
    ...
    def state_action_reward_done(self, rescue_masks):  # 这里返回状态值，奖励值，以及游戏是否结束
        ...
        # 时间惩罚
        time_step_factor = 0.3 # 从0.1升到0.3 优化一下轨迹
        # 发现新区域的奖励系数
        ...
        for i, repetition in enumerate(self.agent_repetition_reward):
            if repetition > self.repetition_threshold_for_reward:     
    ...
    def init_param(self):
        ...
        self.run_time = 240 # 10 25 40 45 55 65 75 100 110 140 200 Run run_time steps per game 当不禁止碰撞的时候，我用的参数是1000
        self.map_size = 60
        self.drone_num = 2
        # Rescue paragrams
        # 已探索区域的比例
        ...
        self.repetition_threshold_for_reward = 4 # 用于指导智能体从死胡同等不好的地方出来
        self.rescue_flag = False
