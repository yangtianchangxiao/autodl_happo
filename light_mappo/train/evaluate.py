...
with open(test_path, 'rb') as tp:
    data = pickle.load(tp)

map_num = len(data)
env = search_grid.SearchGrid(map_set=data, map_num=map_num)
path = r"D:\code\rl_local\RL-in-multi-robot-foraging\marl\mappo_model\cnn_run57_11\actor.pt"
...
actor_1 = actor(all_args, env.observation_space, env.action_space).eval()
actor_1.load_state_dict(checkpoint)
print("actor1")
agents = [actor_1]
