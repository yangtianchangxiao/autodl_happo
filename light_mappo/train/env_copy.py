...
class SearchGrid(gym.Env):
    ...
    def get_drone_obs(self, drone):  # 获得无人机的观测，这里的drone是类
        ...
        # 这是对上面代码的优化
        unobserved_arr = np.array(drone.unobserved)
        obs[unobserved_arr[:, 0], unobserved_arr[:, 1], 0] = 0.5
        obs[unobserved_arr[:, 0], unobserved_arr[:, 1], 1] = 0.5
        obs[unobserved_arr[:, 0], unobserved_arr[:, 1], 2] = 0.5
        ...
        if communicate_list:
            # print("Communicate list is",communicate_list)
            # for x in range(self.map_size):
            #     for y in range(self.map_size):
            #
            #         drone.whole_map[2, x, y] = max(max(self.drone_list[communicate_list[i]].whole_map[2, x, y] for i in range(len(communicate_list))),
            #                                        drone.whole_map[2, x, y])
            #         temp_1 = max(max(self.drone_list[communicate_list[i]].whole_map[1, x, y] for i in range(len(communicate_list))),
            #                                        drone.whole_map[1, x, y])
            #         temp_3 = max(max(self.drone_list[communicate_list[i]].whole_map[3, x, y] for i in range(len(communicate_list))),
            #                                        drone.whole_map[3, x, y])
            #         if drone.whole_map[3, x, y] < temp_3:
            #             drone.grid_communication = drone.grid_communication + 1
            #             drone.whole_map[3, x, y] = temp_3
            #         if drone.whole_map[1, x, y] < temp_1:
            #             drone.obstacle_communication = drone.obstacle_communication + 1
            #             drone.whole_map[1, x, y] = temp_1

            # 这是对上面注释的优化
            maps = np.array([self.drone_list[i].whole_map for i in communicate_list] + [drone.whole_map])
            ...
            drone.grid_communication += np.sum(update_3)
            drone.obstacle_communication += np.sum(update_1)

            drone.whole_map[2, :, :] = np.maximum(max_2, drone.whole_map[2, :, :])
            drone.whole_map[3, :, :] = max_3
            drone.whole_map[1, :, :] = max_1

        # for i in range(sensing_size):  # Fake 在观测范围内进行信息更新，更新时间戳地图1，轨迹地图2和障碍物地图3
        #     for j in range(sensing_size):
        #         x = i + drone.pos[0] - (drone.view_range + index) + 1
        #         y = j + drone.pos[1] - (drone.view_range + index) + 1
        #         for k in range(self.drone_num):
                ...
                for j in range(size):
                    ...
                    if if_obs == True:
                        ...
                        if 0 <= x < self.map_size and 0 <= y < self.map_size:
                            ...
                            self.drone_list[k].whole_map[1, x, y] = 1
    ...
    def state_action_reward_done(self):  # 这里返回状态值，奖励值，以及游戏是否结束
        ...
        # pos_without_change_factor = 2
        # time_step_factor = 0.5
        # average_time_stamp_factor = 3
        ...
        # 这是对上面的优化
        obstacle_pos = np.array(self.obstacles)
        for drone in self.drone_list:
            drone_pos = np.array(drone.pos)
            collision = np.any((drone_pos == obstacle_pos).all(axis=1))
            if collision:
                done = True
        ...
        # print("time step factor,", -time_step_factor)
        for i in range(self.drone_num):
            ...
            # print("agent", i, "information gain", information_gain * (self.obstacle_gain_per_agent[i] - self.last_obstacle_gain_per_agent[i]))
        ...
        # 机器人位置长时间没有移动的惩罚，每隔self.n_step进行一次检测和判断惩罚
        # if self.MC_iter % self.n_step == 0:
        #     if self.last_n_step_pos == None:
        #         self.last_n_step_pos = [[]] * self.drone_num
        #         for i in range(self.drone_num):
        #             self.last_n_step_pos[i] = [self.drone_list[i].pos[0], self.drone_list[i].pos[1]]
        #     else:
        #         dist_list = [0] * self.drone_num
        #         for i in range(self.drone_num):
        #             dist_list[i] = ((self.drone_list[i].pos[0] - self.last_n_step_pos[i][0]) ** 2 + \
        #                             (self.drone_list[i].pos[1] - self.last_n_step_pos[i][1]) ** 2) ** 0.5
        #         if sum(dist_list) / self.drone_num < 3:
        #             reward -= pos_without_change_factor
        #             reward_list = list(map(lambda x: x - pos_without_change_factor, reward_list))
        # print(" - pos_without_change_factor", - pos_without_change_factor)
        # 机器人的每个时间戳平均尽可能大，保证尽可能有多的区域被探索到
        average_list = [0] * self.drone_num
        for i, single_map in enumerate(single_map_set):
            ...
            average_list[i] = self.find_grid_count[i] - self.last_find_grid_cout[i] - self.drone_list[
        ...
        reward_list = [(x + y) * average_time_stamp_factor for x, y in zip(reward_list, average_list)]
