...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        ...
        self.share_obs = np.zeros((self.episode_length + 1, self.n_rollout_threads, *share_obs_shape),
                                  dtype=np.float32)
        # print("self.share_obs.shape",self.share_obs.shape)
        ...
        # self.pred_next_state_feature_orig = None
        # self.
        ...
        self.action_log_probs = np.zeros(
            (self.episode_length, self.n_rollout_threads, num_agents, act_shape), dtype=np.float32)
        self.actions_env = np.zeros(
            (self.episode_length, self.n_rollout_threads, num_agents, 4))  # 4 is the categoriies of actions
        self.rewards = np.zeros(
            (self.episode_length, self.n_rollout_threads, num_agents, 1), dtype=np.float32)
        self.original_rewards = np.zeros(
            (self.episode_length, self.n_rollout_threads, num_agents, 1), dtype=np.float32)
        self.masks = np.ones((self.episode_length + 1, self.n_rollout_threads, num_agents, 1), dtype=np.float32)
        self.bad_masks = np.ones_like(self.masks)
        self.active_masks = np.ones_like(self.masks)
        self.actions_logits_value = np.zeros(
            (self.episode_length, self.n_rollout_threads, num_agents, 4), dtype=np.float32)
        # self.pred_next_state_feature_orig = None
        # self.encode_next_state = None
        # self.pred_action = None
        self.step = 0

    def insert(self, share_obs, obs, rnn_states_actor, rnn_states_critic, actions, action_log_probs,
               value_preds, rewards, masks, actions_logits_value, actions_env, bad_masks=None, active_masks=None,
        ...
        :param actions_env: one-hot code of actions.(20,2,4) (parallel_envs, num_agents, action_categories)
        :param value_preds: (np.ndarray) value function prediction at each step.
        :param rewards: (np.ndarray) reward collected at each step.
        :param masks: (np.ndarray) denotes whether the environment has terminated or not.
        :param actions_logits_value: (np.ndarray) logits value of all the actions
        ...
        #
        #     self.share_obs[self.step + 1] = share_obs.copy().reshape(20, -1, 7500)  # 3*50*50
        # else:
        #     self.share_obs[self.step + 1] = share_obs.copy().reshape(20, -1, 10000)  # 3*50*50

        # 使用CNN时，用这个
        if self.use_centralized_V:
            self.share_obs[self.step + 1] = share_obs.copy() # 3*50*50
        else:
            self.share_obs[self.step + 1] = share_obs.copy()  # 3*50*50
        ...
        # print(rewards,"is this")
        # print("self.step",self.step)+
        self.rewards[self.step] = rewards.copy().reshape(15, -1, 1)
        self.masks[self.step + 1] = masks.copy()
        self.actions_logits_value[self.step] = actions_logits_value.copy()
        self.actions_env[self.step] = actions_env.copy()
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        # print("share_obs shape in shared_buffer", share_obs.shape)
        obs = self.obs[:-1].reshape(-1, *self.obs.shape[3:])
        # print("obs shape in shared_buffer", obs.shape)
        ...
        actions_env = self.actions_env.reshape(-1, self.actions_env.shape[-1])
        actions_logits_value = self.actions_logits_value.reshape(-1, self.actions_logits_value.shape[-1])
        if self.available_actions is not None:
        ...
        for indices in sampler:
            ...
            # 我的评价是，如果使用的是 对indices/agent_num向下取整 应该share_obs就不需要和agent的数量香瓜能力
            # 但是考虑到目前的这个程序相比于原来的尽量不要有太多的改动，那么我们就先试试再说
            share_obs_batch = share_obs[indices // 2] # int(indices/2) 当有3个agent的时候，//3
            rnn_states_batch = rnn_states[indices//2]
            rnn_states_critic_batch = rnn_states_critic[indices//2]
            actions_batch = actions[indices]
            actions_env_batch = actions_env[indices]
            actions_logits_value_batch = actions_logits_value[indices]
            if self.available_actions is not None:
            ...
            yield share_obs_batch, obs_batch, next_obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, \
                  value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, \
                  adv_targ, available_actions_batch, actions_logits_value_batch, actions_env_batch
