...
class Drones(object):
    def __init__(self, pos, view_range, id, map_size):
        ...
        self.whole_map = np.zeros((4, 120, 120), dtype=np.float32)  # 每个机器人保存一个本地地图
...
class SearchGrid(gym.Env):
    def __init__(self, map_set, map_num):
        ...
        self.observation_space = spaces.Box(low=0, high=1, shape=(4 * 120 * 120,))
        self.share_observation_space = spaces.Box(low=0, high=1, shape=(3 * 120 * 120,))
        # When use cnn
        # self.observation_space = spaces.Box(low=0, high=1, shape=(4, 50, 50,))
            ...
            for j in range(obs_size):
                ...
                if 0 <= x <= self.map_size - 1 and 0 <= y <= self.map_size - 1 :  # 是否有障碍物在观测范围内
                    if self.land_mark_map[x, y] == 1:
                        obs[i, j] = 0
                    if self.land_mark_map[x, y] == 2 or ((x, y) in drone_positions and (drone.pos[0]!=x and drone.pos[1]!= y)) :  # 在发现障碍物后对观测进行处理
                        obs[i, j] = 0
    ...
    def init_param(self):
        ...
        self.choose_map = self.map_set[np.random.randint(0, self.map_num)]
        wall = np.argwhere(self.choose_map == 0)  # 获取障碍物的坐标
        free_zones = np.argwhere(self.choose_map == 1)  # 获取空白区域的坐标
        # wall, free_zones = layout.generate_obstacles_and_free_spaces(height=self.map_size, width=self.map_size, obstacle_percentage=self.obstacle_percentage)
